        ...
        if self.instance and self.instance.pk:
            ...
            if request:
                ...
                if 'organizerLink' in self.fields:
                    self.fields['organizerLink'].initial = f"{domain}{reverse('event_manage', kwargs={'event_slug': event_slug, 'organizer_token': organizer_token})}"
                
                if 'registrationLink' in self.fields:
                    self.fields['registrationLink'].initial = f"{domain}{reverse('event_register', kwargs={'event_slug': event_slug})}"
        ...
        if self.instance and self.instance.pk and self.instance.response_token:
            ...
            if self.event and self.request:
                ...
                if 'response_link' in self.fields:
                    self.fields['response_link'].initial = f"{domain}{reverse('response_manage', kwargs={'event_slug': event_slug, 'response_token': response_token})}"
                else:
                    if 'response_link' in self.fields:
